...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="bg-white border border-slate-200/60 rounded-3xl p-8 shadow-sm">
        <form id="article-form" action="{{ route('admin.artikel.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-2 md:col-span-2">
                    ...
                    <input type="text" id="title" name="title" value="{{ old('title') }}" required placeholder="Masukkan judul artikel" class="w-full px-4 py-2.5 rounded-xl border @error('title') border-red-300 focus:border-red-500 @else border-slate-200 focus:border-primary @enderror focus:outline-none text-sm transition-all">
                    @error('title')
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" id="publisher_name" name="publisher_name" value="{{ old('publisher_name', auth()->user()->name) }}" required placeholder="Contoh: Admin Redaksi" class="w-full px-4 py-2.5 rounded-xl border @error('publisher_name') border-red-300 focus:border-red-500 @else border-slate-200 focus:border-primary @enderror focus:outline-none text-sm transition-all">
            ...
            <div class="space-y-2">
                ...
                <div class="flex items-center gap-6">
                    ...
                    <div class="flex-grow">
                        ...
                        <label for="image" class="inline-flex items-center gap-2 px-5 py-3 rounded-xl border border-dashed border-slate-300 text-slate-600 hover:text-primary hover:border-primary hover:bg-primary/5 text-sm font-semibold cursor-pointer transition-all">
                            <i class="fa-solid fa-cloud-arrow-up text-base"></i> Pilih File Gambar Cover
                        </label>
                        <span class="text-3xs text-slate-400 font-medium block mt-2">Format yang didukung: JPEG, PNG, JPG, WEBP, GIF. Ukuran maks: 2MB.</span>
                    </div>
                </div>
                @error('image')
                    <p class="text-xs text-red-500 font-medium mt-1">{{ $message }}</p>
                ...
                <div class="pt-2 space-y-1.5">
                    <label for="image_url" class="text-xs font-bold text-slate-600 block">Atau gunakan URL Gambar</label>
                    <input type="url" id="image_url" name="image_url" value="{{ old('image_url') }}" placeholder="https://images.unsplash.com/photo-..." class="w-full px-4 py-2.5 rounded-xl border @error('image_url') border-red-300 focus:border-red-500 @else border-slate-200 focus:border-primary @enderror focus:outline-none text-sm transition-all">
                    @error('image_url')
                        <p class="text-xs text-red-500 font-medium mt-1">{{ $message }}</p>
                    @enderror
                </div>
